...
- 기존 사방넷 리포트는 사용자가 기간을 고르면 그 기간의 결과를 즉시 계산하는 **조회형 도구**다.
  daou-hub 는 같은 계산 규칙을 하루 단위 RAW로 남기는 **누적형 허브**라서, 30일/90일/1년 추세,
  전후 비교, AI 분석, 운영 판단 기록을 만들 수 있다.
...
- [x] 사방넷 브랜드검색: 계약 고정단가표로 비용 대체
- [ ] 사방넷미니 그 외 DA 배너(블라인드·리멤버·데이블 등): 시트/CSV 소스로 별도 적재
- [ ] 다우오피스 사람인(업로드 매체): 시트/CSV 소스로 별도 적재 (구글·메타는 API로 커버)
- [ ] 수기 엑셀 1일치와 대조해 광고비·가입 숫자 일치 확인

## 매체별 수집 방식 (앱 기준)
- 사방넷: 네이버·구글·메타 API + DA배너(업로드)
- 애드콘·엔팩스: 네이버·구글 API (메타 없음)
- 다우오피스·HR: 구글·메타 API + 네이버·사람인(업로드), 가입=GA4
